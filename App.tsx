...
function BlueprintStudio() {
  ...
  const addNode = (type: NodeCardType) => {
    ...
    if (type === NodeCardType.CONTAINER) {
      newNode.zIndex = -1;
      newNode.style = { width: 300, height: 200 };
      newNode.data.containerStyle = { backgroundColor: '#f8fafc', borderStyle: 'dashed', borderColor: '#cbd5e1' };
    }
  ...
  const exportToExcel = () => {
    ...
    projects.forEach(p => {
      ...
      allNodes.push(...data.nodes.map(n => ({ 
        ...
        ContainerID: n.data.containerId || '',
        ContainerStyle: n.data.containerStyle ? JSON.stringify(n.data.containerStyle) : ''
      })));
        ...
        importedProjects.forEach(p => {
          ...
          newMap[pId] = {
            nodes: nodesRaw.filter(n => n.ProjectID === pId).map(n => {
              const isContainer = n.Type === NodeCardType.CONTAINER;
              return {
            ...
            <div className="relative flex-shrink-0">
              ...
              {openMenuType === 'add' && (
                <div className="absolute top-full left-0 mt-3 w-56 bg-white rounded-2xl shadow-2xl border border-slate-100 py-2 overflow-hidden animate-in fade-in slide-in-from-top-2 duration-200 z-50">
                  ...
                  <button onClick={() => addNode(NodeCardType.CONTAINER)} className="w-full flex items-center gap-3 px-4 py-3 text-sm font-bold text-slate-700 hover:bg-slate-50 hover:text-slate-900 transition-colors"><div className="p-1.5 bg-slate-100 text-slate-600 rounded-lg"><PackageOpen size={16} /></div><span className="text-left">{t('container') || 'Container'}</span></button>
                </div>
              )}
            </div>
