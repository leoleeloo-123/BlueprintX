...
export const BlueprintCard = memo(({ data, id, selected }: NodeProps<NodeData & { settings: GlobalSettings }>) => {
  ...
  const isContainer = data.cardType === NodeCardType.CONTAINER;
  
  if (isContainer) {
    const style = data.containerStyle || { backgroundColor: '#f8fafc', borderStyle: 'dashed', borderColor: '#cbd5e1' };
    return (
      <div 
      ...
      >
        <div className="flex items-center justify-between px-4 py-3 border-b" style={{ borderColor: style.borderColor, backgroundColor: 'rgba(255,255,255,0.5)' }}>
          <h3 className="font-bold text-slate-700 text-lg">{data.label}</h3>
          <div className="flex items-center gap-1">
            <button onClick={() => data.onEdit?.(id)} className="p-1.5 text-slate-400 hover:text-blue-600 hover:bg-white rounded transition-colors"><Edit3 size={16} /></button>
            <button onClick={() => data.onDuplicate?.(id)} className="p-1.5 text-slate-400 hover:text-blue-600 hover:bg-white rounded transition-colors"><Copy size={16} /></button>
            <button onClick={() => data.onDelete?.(id)} className="p-1.5 text-slate-400 hover:text-red-600 hover:bg-white rounded transition-colors"><Trash2 size={16} /></button>
          </div>
        </div>
