...
export const EditorModal: React.FC<EditorModalProps> = ({ node, settings, onClose, onSave, language }) => {
  ...
  const isContainer = cardType === NodeCardType.CONTAINER;

  const [containerStyle, setContainerStyle] = useState(node.data.containerStyle || {
    ...
    <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-sm z-50 flex items-center justify-center p-4">
      <div className="bg-white w-full max-w-2xl rounded-2xl shadow-2xl overflow-hidden flex flex-col max-h-[90vh]">
        ...
        <div className="p-6 overflow-y-auto flex-1 space-y-6 custom-scrollbar">
          {isContainer ? (
            <div className="space-y-4">
              <div>
                <label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1.5">Container Name</label>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1.5">Background Color</label>
                  <input type="color" value={containerStyle.backgroundColor} onChange={e => setContainerStyle({...containerStyle, backgroundColor: e.target.value})} className="w-full h-10 rounded cursor-pointer" />
                </div>
                <div>
                  <label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1.5">Border Color</label>
                  <input type="color" value={containerStyle.borderColor} onChange={e => setContainerStyle({...containerStyle, borderColor: e.target.value})} className="w-full h-10 rounded cursor-pointer" />
                </div>
                <div className="col-span-2">
                  <label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1.5">Border Style</label>
                  <select value={containerStyle.borderStyle} onChange={e => setContainerStyle({...containerStyle, borderStyle: e.target.value as any})} className="w-full px-4 py-2 bg-slate-50 border border-slate-200 rounded-xl text-sm font-semibold focus:ring-2 focus:ring-blue-500 outline-none">
        ...
        <div className="p-6 bg-slate-50 border-t border-slate-100 flex justify-end gap-3">
          ...
          <button onClick={() => onSave({ label, categoryId, columns, description, bulletPoints, comment, dataSourceId, tags: assignedTags, containerStyle })} className="px-8 py-2 bg-slate-900 text-white rounded-xl text-sm font-bold shadow-lg">{t('commit_update')}</button>
        </div>
      </div>
    </div>
  );
